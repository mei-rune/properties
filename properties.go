...
func ReadFile(nm string) (map[string]string, error) {
	f, e := os.Open(nm)
	if nil != e {
		return nil, e
	}
...
func ReadWith(r io.Reader, cb func(key, value string), logf func(fmtStr string, args ...interface{})) error {
	lineCount := 0
	...
	for scanner.Scan() {
		...
		if !hasEqualChar {
			if logf != nil {
				logf("行 %d 格式不正确, 值没有等号 - %s", lineCount, txt)
			}
			continue
		}
